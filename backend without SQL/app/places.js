...
router.get("/", async (req, res) => {
  const result = await fileDb.getItems('places');
  const response = [];
...
router.get("/:id", async (req, res) => {
  const result = await fileDb.getItemById(req.params.id, 'places');
  res.send(result);
});
...
router.post("/", async (req, res) => {
  ...
  if (!response.name || response.name.length === 0) {
    ...
    await fileDb.addItem(response, 'places');
    res.send(response);
  }
});
...
router.put("/:id", async (req, res) => {
  ...
  if (!response.name || response.name.length === 0) {
   ...
   const result = await fileDb.editItem(req.params.id, response, 'places');
   result === -1 
...
router.delete("/:id", async (req, res) => {
  const result = await fileDb.deleteItem(req.params.id, 'places');
  if (result === -1) {
    res.status(404).send({error: 'Not found'})
  } else if (!result) {
